...
public class IntegrationTest {
    ...
    private static Config nodeConfig(int grcpPort, List<Integer> managementPorts, int managementPortIndex) {
        return ConfigFactory.parseString(
                "shopping-cart-service.grpc.port = " + grcpPort + "\n" +
                        "akka.management.http.port = " + managementPorts.get(managementPortIndex) + "\n" +
                        "akka.discovery.config.services.shoppingcartservice.endpoints = [\n" +
                        "  { host = \"127.0.0.1\", port = " + managementPorts.get(0) + "},\n" +
                        "  { host = \"127.0.0.1\", port = " + managementPorts.get(1) + "},\n" +
                        "]");
    }

    static private class TestNodeFixture {
        ...
        public TestNodeFixture(int grcpPort, List<Integer> managementPorts, int managementPortIndex) {
            ...
            clientSettings = GrpcClientSettings.connectToServiceAt("127.0.0.1", grcpPort, system)
                    .withTls(false);
        }
    ...
    public static void setup() throws Exception {
        List<InetSocketAddress> inetSocketAddresses =
                CollectionConverters.SeqHasAsJava(SocketUtil.temporaryServerAddresses(6, "127.0.0.1", false))
                        .asJava();
        List<Integer> grpcPorts = inetSocketAddresses.subList(0, 3)
                .stream()
                .map(InetSocketAddress::getPort)
                .collect(Collectors.toList());
        List<Integer> managementPorts = inetSocketAddresses.subList(3, 6)
                .stream()
                .map(InetSocketAddress::getPort)
                .collect(Collectors.toList());
        ...
        testOrderService = new ShoppingOrderService() {
            @Override
        ...
        systems.stream().forEach(system -> {
            upProbe.awaitAssert(() -> {
                ...
                cluster.state().getMembers().iterator().forEachRemaining(member ->
                        assertEquals(MemberStatus.up(), member.status())
                );
                return null;
            });
        });
