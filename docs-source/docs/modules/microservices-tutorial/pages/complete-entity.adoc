...
On this page, we will complete the Cart `ShoppingCart` entity with more commands and events. `ShoppingCart` will use Event Sourcing to persist events that represent changes to the state of the cart.
...
ifdef::review[REVIEWERS: it was just way too confusing to describe in detail the optional operations before the reader had gone through the "required" operations. I moved it to the end.]

== Source downloads

ifdef::review[REVIEWERS: the title over the tabs looked strange without some introductory text. I like this, if you like it, I'll make the others the same way.]
...
ifdef::review[REVIEWERS: the service descriptor refers to GetCart, but in the source code examples above the name is Get?]
