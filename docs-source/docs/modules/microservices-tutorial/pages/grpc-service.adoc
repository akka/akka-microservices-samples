...
****
You can download the initial project template link:_attachments/0-shopping-cart-start-scala.zip[here] or use `sbt` to start a new project based on the template as explained in xref:template.adoc[Run the template].

The completed source code for this section can be downloaded link:_attachments/1-shopping-cart-grpc-scala.zip[here].
****
