...
=======
* [.group-scala]#link:_attachments/0-shopping-cart-start-scala.zip[Source]# [.group-java]#link:_attachments/0-shopping-cart-start-java.zip[Source]# with the initial project template. Alternatively, use `sbt` to start a new project as explained in xref:template.adoc[Run the template].
