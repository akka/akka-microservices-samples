...
Read the xref:deployment:index.adoc[overview of Kubernetes deployment] to get an understanding of how the Kubernetes deployment with the Akka Platform Operator works.

Create a xref:deployment:aws-install.adoc[Kubernetes cluster and install the Akka Platform Operator].

=== Build Docker image

Create a Docker repository and authenticate Docker as described in xref:deployment:aws-ecr.adoc[Amazon Docker repository].
...
In `kubernetes/shopping-cart-service-cr.yml` you find the deployment descriptor of the `AkkaMicroservice` that the Akka Platform Operator will use when deploying the application in Kubernetes.
...
The Akka Platform Operator will notice the deployment descriptor and deploy the `shopping-cart-service`. You can see progress by viewing the status:
