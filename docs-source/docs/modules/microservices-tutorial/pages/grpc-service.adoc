...
****
You can download the link:_attachments/0-shopping-cart-start-scala.zip[initial project template] or use `sbt` to start a new project as explained in xref:template.adoc[Run the template].

Or, if you prefer, you can download the link:_attachments/1-shopping-cart-grpc-scala.zip[completed source] for this tutorial step.
****
