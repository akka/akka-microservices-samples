...
[.group-java]
****
This example uses Spring Data and Hibernate to access the database.
...
[.group-scala]
****
This example uses the link:http://scalikejdbc.org/[ScalikeJDBC library {tab-icon}, window="tab"] to access the database via JDBC. **You may use any other JDBC library to achieve this.**

The code to integrate with ScalikeJDBC is `DBsFromConfig`, `ScalikeJdbcSession` and `ItemPopularityRepositoryImpl`.
****
...
----
include::example$shopping-cart-service-scala/src/main/scala/shopping/cart/repository/ItemPopularityRepository.scala[tag=trait]
----

.  [.group-java]#Use the Spring `ApplicationContext` to retrieve the implementation for the repository# [.group-scala]#Add the implementation for Postgres by using the `ScalikeJdbcSession`:#
+
[.tabset]
Java::
+
...
[source,scala,indent=0]
----
include::example$shopping-cart-service-scala/src/main/scala/shopping/cart/repository/ItemPopularityRepository.scala[tag=impl]
----
...
[source,scala,indent=0]
----
include::example$shopping-cart-service-scala/src/main/scala/shopping/cart/ItemPopularityProjectionHandler.scala[tags=handler]
----
+
