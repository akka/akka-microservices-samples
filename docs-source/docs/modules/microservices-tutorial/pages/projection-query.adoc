...
----
+
<1> `tagsFor` to assign the `projectionTag`
...
<2> use the `ActorSystem` to get an instance of an `Executor` tailored for blocking operations
<3> Implement `getItemPopularity` by calling the repository to find the projected model by id. Note that calls to repository are blocking therefore we wrap the call using `CompletableFuture.supplyAsync` and we use the blocking executor to run it.
...
[source,sql,indent=0]
----
include::example$04-shopping-cart-service-java/ddl-scripts/create_user_tables.sql[tag=create]
----
