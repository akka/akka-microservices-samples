...
In this part, we will create an {akka-projection}/[Akka Projection {tab-icon}, window="tab"] from the events of the `ShoppingCart` entity. The Projection updates counts in the database to keep track of how popular the items are. The database can be queried for how popular an item is, which is a query that the `ShoppingCart` entities themselves can't answer since they can only be addressed by individual cart identifiers.
...
Read the xref:concepts:cqrs.adoc[CQRS] section as a background to why it is good to build a query representation from the events of the entities with a Projection. The {akka-blog}/news/2020/09/10/akka-projection-intro-video[Introduction to Akka Projections video {tab-icon}, window="tab"] is also a good starting point for learning Akka Projections.
...
<1> extends [.group-scala]#`akka.projection.scaladsl.Handler`# [.group-java]#`akka.projection.javadsl.Handler`#
...
You can learn more about the `CassandraSession` in the {alpakka}/cassandra.html[Alpakka reference documentation {tab-icon}, window="tab"].
...
The tag is assigned to the `EventSourcedBehavior`. [.group-scala]#In the `ShoppingCart.apply` method, add the `projectionTag` parameter and pass it to `.withTagger`:# [.group-java]#In the `ShoppingCart` constructor, add the `projectionTag` parameter and use it to override the `tagsFor` method:#
...
Here we use five different tags. The tagging is not easy to change later without system downtime. Before going live in production you should consider how many tags to use, see {akka-projection}/running.html[Akka Projections reference documentation {tab-icon}, window="tab"] for more information.
