...
Commands are the "external" API of an entity. Entity state can only be changed by commands. The results of commands are emitted as events. A command can request state changes, but different events might be generated depending on the current state of the entity. To add a command and an event, follow these steps:
...
Items added to the Cart are added to a `Map`. The contents of the `Map` comprise the Cart's state. Add the `Map` to the `ShoppingCart` object as shown:
...
ifdef::review[REVIEWERS: The first sentece says events "are the only way to change" state. However, previously we said events don't change anything, they are only a record of facts, something that has happened? Also confusing is the statement that "Other parties can't send events" Clearly other services can also send events, just not events about this entity's state, right? Once I get answers to these questions I will have suggestions about clarifying these paragraphs.]
...
ifdef::review[REVIEWERS: what is "this" in the first sentence? The command handler, event handler, etc?]

To glue this together we need some initialization code. Our code will distribute nodes in the Akka Cluster with https://doc.akka.io/docs/akka/current/cluster-sharding.html[Cluster Sharding {tab-icon}, window="tab"], enable snapshots to reduce recovery time when the entity is started, and restart with backoff in the case of failure. 
...
ifdef::todo[TODO: add Java]

Then we need to call [.group-scala]#`ShoppingCart.init` from `Main.scala`#:
...
ifdef::review[REVIEWERS: If I'm reading the rest of this section correctly, the user doesn't need to add anything here because it's included in the template and conf files? If so, I think we need to state that more clearly.]
...
We want to send commands to the entities from the gRPC service implementation. In the xref:grpc-server.adoc[previous step], we wrote a dummy implementation of `addItem` in the `ShoppingCartServiceImpl`. We can now replace that and send `ShoppingCart.AddItem` commands from `ShoppingCartServiceImpl`:
...
Also, we included the new `ActorSystem` parameter to the constructor when creating an instance of `ShoppingCartServiceImpl` from the `ShoppingCartServer`.

== Cassandra

The events are stored in Cassandra and the template project includes configuration for that in the `src/main/resources/persistence.conf` file. We have to enable this configuration by including `persistence.conf` in `application.conf`:
...
To run the service, we first need to start Cassandra to persist the events. Then we can run the service:
