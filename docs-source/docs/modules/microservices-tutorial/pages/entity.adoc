= IV: Create Event Sourced Cart entity
:toc:
...
ifdef::review[REVIEWERS: re: bullet 3 above, I didn't see any explanation of how to distribute entities in the rest of the page. Maybe because it's part of deploying in the cloud, which isn't written yet? But isn't this something that we should introduce and motivate in the overview of this page? That is why it's a good think to distribute the entities and how Akka makes it easy for you to do with cluster sharding?]
