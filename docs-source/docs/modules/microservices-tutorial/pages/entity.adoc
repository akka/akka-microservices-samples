...
Create a xref:deployment:aws-install.adoc[Kubernetes cluster and install the Akka Platform Operator] if you haven't already.
...
Create a Docker repository and authenticate Docker as described in xref:deployment:aws-ecr.adoc[Amazon Docker repository] if you haven't already.
...
<2> Add a `jdbc.credentialsSecret` section pointing to the secret created in the xref:deployment:aws-rds.adoc[Amazon RDS] instructions. 
