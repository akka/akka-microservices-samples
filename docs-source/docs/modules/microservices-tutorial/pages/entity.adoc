...
[source,hocon]
----
include::example$02-shopping-cart-service-scala/src/main/resources/persistence.conf[tag=persistence-cassandra-config]
----
...
* When loading the `create_tables.cql` file, make sure to use the same name as your running Cassandra container name. The container name is not fixed and depends on the parent folder of the docker-compose file. The above example assumes the project was created using the seed template and named `shopping-cart-service`.
