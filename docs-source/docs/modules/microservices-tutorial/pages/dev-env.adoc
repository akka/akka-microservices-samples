...
This tutorial makes used of Cassandra and Kafka. You may use local installations during development, but we recommend run both servers using https://docs.docker.com[Docker]. The template project includes a https://docs.docker.com/compose/[Docker Compose] file on its root you can use to bootstrap Cassandra, Kafka and Zookeeper (the latter required by the Kafka server).

After installing `docker` and `docker-compose`, you can run the following command at the root of the project.
...
Create the Cassandra keyspace and tables with:

----
docker exec -i shopping-cart-service_cassandra_1 cqlsh -t < ddl-scripts/create_tables.cql
----

This CQL script is located inside the `ddl-scripts` at the root of the project. It will create the keyspace and all tables needed for Akka Persistence as well as the offset store table for Akka Projection.

[CAUTION]
====
* The keyspace as created by the script is probably not what you need in a production environment. This is good enough for local development though.
====
