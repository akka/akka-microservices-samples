...
ifdef::review:[REVIEWERS: I'm assuming you will also supply instructions for Java? I've organized so that would be easy to do.]
...
This tutorial uses Cassandra and Kafka. You may run local installations during development, but we recommend running both services using https://docs.docker.com[{tab-icon}, window="tab"]. The template project includes a https://docs.docker.com/compose/[Docker Compose]  https://docs.docker.com/compose/[{tab-icon}, window="tab"] file on its root you can use to run Cassandra and Kafka. The provided Docker Compose file also runs Zookeeper which is required to run a Kafka broker.
...
. https://www.jetbrains.com/idea/download/[install IntelliJ]
. From the *Preferences* menu, add the `sbt` plugin.
...
The https://github.com/fullstorydev/grpcurl[grpcurl {tab-icon}, window="tab"] tab is useful for making gRPC calls from the command line. It is used in tutorial procedures and we recommend that you install it as described in https://github.com/fullstorydev/grpcurl#installation[the grpcurl documentation {tab-icon}, window="tab"].
...
ifdef::review:[REVIEWERS: in the new UI template that we will have soon, each page will have a next and previous link with the full page title, which will eliminate the need for the Learn More section, unless you have links other than to the next and prev that you want to supply. I've left them in for now.]
