...
This guide assumes you created a new project using the xref:microservices-tutorial:template.adoc#seed-template[Akka Microservicies template].
...
The project produced with the xref:microservices-tutorial:template.adoc#seed-template[Akka Microservicies template] assumes the application will use the {akka-persistence-cassandra}[Akka Persistence Cassandra plugin] {akka-persistence-cassandra}[{tab-icon}, window="tab"]. You can opt out from using that plugin and use the {akka-persistence-jdbc}[Akka Persistence JDBC] {akka-persistence-jdbc}[{tab-icon}, window="tab"] or other plugins instead.
...
The project produced with the xref:microservices-tutorial:template.adoc#seed-template[Akka Microservicies template] assumes the application will use a Cassandra database to store the {akka-projection}/overview.html#overview[offset tracking] {akka-projection}/overview.html#overview[{tab-icon}, window="tab"]  required by the projections. You can replace the artifact to use an RDBMS:
...
The project produced with the xref:microservices-tutorial:template.adoc#seed-template[Akka Microservicies template] projects data include the necessary dependencies to project data to a Kafka topic. If you are not projecting data into Kafka, remove the version declaration and the artifact:
