...
See the xref:shopping-microservices:entity.adoc[step adding an Event Sourced entity], and the xref:shopping-microservices:complete-entity.adoc[step completing the Event Sourced entity] on the tutorial for more details.
...
If you prefer Availability to Consistency, you may also use converging algorithms so concurrent modifications on multiple state instances in memory converge into a single, valid state.

ifdef::todo[TODO: link to Akka's Replicated Event Sourcing docs]
