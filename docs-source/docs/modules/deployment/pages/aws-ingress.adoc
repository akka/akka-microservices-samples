...
When the preceding command is successful, it returns metadata about the uploaded certificate, including its Amazon Resource Name (ARN). Save the ARN for the next step.
...
spec:
  ...
  grpcIngress:
    enabled: true
    certificate: "arn:aws:iam:eu-central-1:803424716218:server-certificate/MyCertificate"
    class: "alb"
----
