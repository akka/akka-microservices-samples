...
To expose gRPC or HTTPS to the internet you can ues the https://kubernetes-sigs.github.io/aws-load-balancer-controller/latest/[AWS Load Balancer Controller {tab-icon}, window="tab"] with a https://kubernetes.io/docs/concepts/services-networking/ingress/[Kubernetes Ingress {tab-icon}, window="tab"] created by the Akka Platform Operator.
...
Add the `grpcIngres` section to the CR:
...
When the CR has been applied the Akka Platform Operator will create an Ingress and you can retrieve the public address with:

[source,shell script]
----
kubectl get ingress shopping-order-service-grpc-in
----
