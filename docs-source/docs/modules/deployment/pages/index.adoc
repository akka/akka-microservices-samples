...
The Akka Cloud Platform is currently available for deploying on Amazon Elastic Kubernetes Service (AKS). It is available through the https://aws.amazon.com/marketplace/pp?sku=7hhouu843kzmgqs6besqjjsbf[Amazon Marketplace {tab-icon}, window="tab"] to install on your Amazon EKS cluster.

Through the Akka Operator, the Akka Cloud Platform provides the following high level features:
...
To use the Akka Cloud Platform, you first need to install prerequisites and install the Akka Operator in an EKS cluster. The Akka Operator manages Akka Microservices in all namespaces of the Kubernetes cluster, which means that you should only install the operator once. It is possible to configure the operator to only manage a specific namespace, if required.
...
. The Akka Cloud Platform sees the new deployment descriptor and creates a https://kubernetes.io/docs/concepts/extend-kubernetes/api-extension/custom-resources/[Kubernetes Deployment {tab-icon}, window="tab"]
