...
There is an array of options for creating a Cloud SQL instance with various different network configurations. This guide demonstrates:
...
For production consider using the https://cloud.google.com/sql/docs/postgres/connect-overview[Cloud SQL proxy and IAM database authentication].
Direct connectivity between GKE and Cloud SQL via private IP is only possible if using a https://cloud.google.com/kubernetes-engine/docs/how-to/alias-ips[Native VPC cluster] otherwise the CLoud SQL proxy is required.
...
You may need the help of an admin for your GCP project to get the correct permissions for automatically allocating the IP range. This only needs to be done once
per network.
