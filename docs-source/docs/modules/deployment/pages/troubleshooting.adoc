...
- `UP` displays how many replicas are in normal operating state. 

If `REPLICAS` equals `UP` then all the Akka cluster is up and running without any issues on all the allocated pods. If `REPLICAS` is bigger than `UP` then not shown by default `NON-UP` and `UNREACHABLE` will provide details on how many replicas are not in normal operating state or unreachable.
...
In this mode, additionally to `UP` there are also `NON-UP` and `UNREACHABLE` that display how many replicas are non-up or unreachable respectively.

- `NON-UP` displays how many replicas are NOT in normal operating state, e.g. leaving, joining or down. 
- `UNREACHABLE` displays how many replicas are in unreachable state.
...
shopping-cart-service   DeploymentFailed   2          2         7h25m 1     0        0            akka.operator.action.ActionException: Action [GetAction] failed: jdbc Secret [shopping-cart-service-jdbc-zecret] doesn't exist
----
