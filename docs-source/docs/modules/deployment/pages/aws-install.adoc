...
If you EKS cluster has not been created with https://docs.aws.amazon.com/eks/latest/userguide/enable-iam-roles-for-service-accounts.html[IAM OIDC provider {tab-icon}, window="tab"] (e.g. with the `--with-oidc` option with `eksctl`) then enable with:
[source,shell script]
----
...
[source,shell script]
----
aws iam create-policy --policy-name AkkaPlatformOperator --policy-document file://policy
----
...
[source,shell script]
----
helm install akka-operator target/akka-operator-<version>.tgz \
  --namespace lightbend  \
