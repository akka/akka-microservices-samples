...
[source,shell script]
----
eksctl utils associate-iam-oidc-provider --cluster eks-akka-demo --approve
----
...
----
+
Optionally pick a version to install, or omit the `helm install --version` parameter to install the latest version.
+
[source,shell script]
----
