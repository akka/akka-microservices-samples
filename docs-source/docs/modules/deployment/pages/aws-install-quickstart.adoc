...
4. (Optional) Setup an Amazon MSK cluster
5. (Optional) Setup an Amazon Aurora RDS cluster
...
The fastest way to get started with Akka Cloud Platform is to run the Akka Cloud Platform Deployment Pulumi playbook.
...
To setup your local `kubectl` configuration you can obtain the Pulumi `kubeconfig` output by exporting it to a file and defining the `KUBE_CONFIG` environment variable:
...
kubectl run -i --tty rds-mgmt --image=postgres --restart=Never --rm \
  --env=PGPASSWORD=$(pulumi stack output dbPassword --show-secrets) -- \
  psql -h $(pulumi stack output dbEndpoint) -U acpadmin -d acp
----
...
* `kubeconfig` - Complete contents of a `KUBE_CONFIG` configuration for the provisioned cluster
* `operatorNamespace` - Kubernetes namespace where the Akka Cloud Platform operator is running

Amazon Aurora Cluster

* `dbClusterId` - Cluster ID
* `dbName` - Default database created for Akka Cloud Platform
* `dbUsername` - Master username
* `dbPassword` - Master password. This is redacted in standard console output. To retrieve the value use the `pulumi stack output` command. For example, to assign the password to an environment variable: `DB_PASSWD=$(pulumi stack output dbPassword --show-secrets)`.
* `dbEndpoint` - Database writer endpoint hostname
* `dbReaderEndpoint` - Database reader endpoint hostname
* `dbSecret` - The Kubernetes Secret (in `operatorNamespace` namespace) containing relational database connection details.
