...
For example, you might use different instances to represent different environments such as `dev`, `testing`, `staging`.
By default state is managed using Pulumi's state management service (a remote service), but you can choose other a https://www.pulumi.com/docs/intro/concepts/state/#backends[self-managed backend {tab-icon}, window="tab"] if you like.
Using Pulumi's service is the fastest way to get started and provides several other benefits such as sharing state with others, view progress of deployment, view output variables and resulting cloud resources that were created.
...
Pulumi will reconcile state from the playbook to your desired target environment.
When you run a `pulumi up` (update) all the resources defined in the playbook will be provisioned.
...
While the cluster is provisioning you will be informed at the terminal about which resources are being created or are created.
If using the the Pulumi service for state then the `pulumi up` command will include a link to "View Live" the progress of the deployment.
The `pulumi up` command will exit when all resources are fully deployed and online.
If a failure occurs then the command will exit early and inform you of the reason.
If this occurs and you can correct the issue yourself then rerun `pulumi up` to resume updating where you left off.
If the error can't be resolved then you can use the `pulumi destroy` command to rollback any partially deployed resources.

If the deployment is successful then all the pertinent details of the cluster will be output to the console (except for passwords).
...
Do you want to perform this update? yes
     ...
     Type                                                                 Name                                                                                        Status
 +   pulumi:pulumi:Stack                                                  akka-cloud-platform-deploy-seglo                                                            created

...
----

When the stack is deployed successfully you will have a list of outputs presented. For example:

[source,shell script]
----
Outputs:
    clusterName                : "acp-<STACK NAME>-eks-eksCluster-1550877"
    ...
    jdbcUsername               : "acpadmin"
    kafkaBootstrapBrokers      : "b-1.acp-<STACK NAME>-msk-b59fc25.fx64yv.c5.kafka.us-west-2.amazonaws.com:9092,b-2.acp-<STACK NAME>-msk-b59fc25.fx64yv.c5.kafka.us-west-2.amazonaws.com:9092"
    kafkaBootstrapBrokersTls   : "b-1.acp-<STACK NAME>-msk-b59fc25.fx64yv.c5.kafka.us-west-2.amazonaws.com:9094,b-2.acp-<STACK NAME>-msk-b59fc25.fx64yv.c5.kafka.us-west-2.amazonaws.com:9094"
    kafkaBootstrapServerSecret : "acp-<STACK NAME>-kafka-secret"
    kafkaZookeeperConnectString: "z-1.acp-<STACK NAME>-msk-b59fc25.fx64yv.c5.kafka.us-west-2.amazonaws.com:2181,z-2.acp-<STACK NAME>-msk-b59fc25.fx64yv.c5.kafka.us-west-2.amazonaws.com:2181,z-3.acp-<STACK NAME>-msk-b59fc25.fx64yv.c5.kafka.us-west-2.amazonaws.com:2181"
    kubeconfig                 : {
		...
    }
...
A full list of xref:deployment:aws-install-quickstart.adoc#output-variables[output variables].
...
kubectl get secret $KAFKA_SECRET -n $OPERATOR_NAMESPACE -o yaml | \
	sed s/"namespace: $OPERATOR_NAMESPACE"/"namespace: $SERVICE_NAMESPACE"/ | \
	kubectl apply -n $SERVICE_NAMESPACE -f -
----
====
...
OPERATOR_NAMESPACE=$(pulumi stack output operatorNamespace)
SERVICE_NAMESPACE=<SERVICE NAMESPACE>
KAFKA_SECRET=$(pulumi stack output jdbcSecret)
kubectl get secret $KAFKA_SECRET -n $OPERATOR_NAMESPACE -o yaml | \
	sed s/"namespace: $OPERATOR_NAMESPACE"/"namespace: $SERVICE_NAMESPACE"/ | \
	kubectl apply -n $SERVICE_NAMESPACE -f -
----
