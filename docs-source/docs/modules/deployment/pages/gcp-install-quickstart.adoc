...
* Pulumi requires that you setup default application credentials in order to interact with your Google Cloud resources. Pulumi requires this.
...
* Some optional settings can be set up to provide ready defaults. The following environment variables can be set to provide optional configurations to the Pulumi Google Cloud Platform provider:
...
The fastest way to get started with Akka Cloud Platform on GCP is to run the Akka Cloud Platform Deployment Pulumi playbook, which contains a bunch of instructions that plays like a book when run and on successful completion provisions the cluster and all the appropriate artifacts.
Here are the steps to follow to have your playbook up 'n running:
...
NOTE: Pulumi has an issue with tracking ready status for containers with sidecars. In GCP implementation of the akka platform operator, ubbagent (the billing agent)
...
Doing a `pulumi cancel` will cancel the running update. However in some cases the update might have gone __'OK'__ despite the message mentioned above. In that case, `pulumi cancel` gives the following message __The Update has already completed__.
