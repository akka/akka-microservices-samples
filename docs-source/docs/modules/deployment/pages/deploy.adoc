...
It assumes that you have already read the xref:deployment:index.adoc[overview of Kubernetes deployment], created a xref:deployment:aws-install.adoc[Kubernetes cluster and installed the Akka Platform Operator].

:sectnums:
== Build Docker image

Build and publish a Docker image of the application as described in xref:deployment:aws-ecr.adoc[Amazon Docker repository].

== Update the deployment descriptor

Update the deployment descriptor in `kubernetes/shopping-cart-service-cr.yml` of the `AkkaMicroservice` that the Akka Platform Operator will use when deploying the application in Kubernetes.
