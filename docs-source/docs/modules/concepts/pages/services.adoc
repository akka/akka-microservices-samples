...
Service Discovery look-ups hit a Service Registry that keeps an up-to-date list of addresses providing a given service. In Kubernetes, for example, when a Pod is xref:how-to:health-checks.adoc[ready] the DNS registry is updated, so a new Service Discovery look-up for that service can now return this new Pod address and port.
