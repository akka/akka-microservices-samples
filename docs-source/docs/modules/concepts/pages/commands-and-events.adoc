...
Once the state reconstructed finishes, the entity is ready to process new commands. A command is a demand to mutate the state.

Events will be serialized and published into the _Journal_ table on the database. The _Journal_ can then be consumed by the emitting entity or by third parties (e.g. to build an aggregate or other kinds of projections). Events are built on top of xref:message-driven-event-driven.adoc#message_vs_event[messages].
