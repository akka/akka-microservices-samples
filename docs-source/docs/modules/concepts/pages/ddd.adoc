...
It models a central concept in your system having a prominet domain object that gives name to it. We call that object the Aggregate Root. 

We can take a shopping cart application as an example. A shopping cart aggregate may contain different items. An item itself is a domain concept, but the shopping cart is the central domain concept in that case. The shopping cart is therefore the Aggregate Root and gives name to the Aggregate. We can simply talk about the Shopping Cart Aggregate. 
