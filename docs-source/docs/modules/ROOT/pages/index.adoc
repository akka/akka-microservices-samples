...
- Operator — Operator management is key. You will find details on xref:deployment:aws-install.adoc[installing]
...
- TLS Certificates — Users often need information about TLS Certificates. You will find the pertinent information within the xref:deployment:aws-ingress.adoc#_tls_certificate[AWS LoadBalancer Controller] page.
...
- License expiration and renewal — You may have purchased your license from the AWS Marketplace, or  directly from Lightbend. In either case the license expiration and renewal for the Akka Cloud Platform is handled as follows:
* If the Akka Cloud Platform license expires without being renewed, your current application will continue to run, but you will no longer be able to deploy.
